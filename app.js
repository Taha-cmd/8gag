			...
			function (err, result) {
				if (err) {
					...
					if (loggedIn)
						...
						res.render("boards", {
							data: { loggedIn: false, menu: menuNotLoggedIn, boards: result },
						});
					connection.close();
				}
			}
					...
					if (loggedIn)
						res.render("board", {
							data: {
								...
								uid: sess.uid,
								board: result,
								boardName: queryObject["boardName"],
								boardID: queryObject["board"],
								username: sess.username,
							},
						});
					else
	...
	try {
		...
		if (loggedIn) {
			await connection2.execute(
				'select "board_id" from "subscription" where "user_id" = :id',
				[res_user.rows[0]["id"]],
				function (err, result) {
...
app.post("/uploadBoardPost", async function (req, res) {
	...
	try {
		...
		var st =
			'insert into "board_post" ("content", "user_id", "board_id") values (\'' +
		...
		await connection.execute(
			...
			function (err, result) {
				if (err) {
					...
					res.redirect(
						"http://127.0.0.1:8080/board?board=" +
							req.body.board_id +
							"&boardName=" +
							req.body.board_name
					);
...
app.post("/sub", async function (req, res) {
	...
	try {
		...
		var st =
			'insert into "subscription" ("user_id", "board_id") values (' +
			sess.uid +
			", " +
			req.body.board_id +
			")";
			...
			function (err, result) {
				if (err) {
					...
					for (var i = 0; i < sess.subs.length; i++)
						if (sess.subs[i] == Number(req.body.board_id)) sess.subs[i] = -1;
